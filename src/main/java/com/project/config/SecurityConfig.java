...
public class SecurityConfig {
	...
	SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception {
		...
		// 5. 로그아웃 설정 수정
		/*
		 * httpSecurity.logout(logout -> logout .logoutUrl("/logout") // 로그아웃을 처리할 URL
		 * (기본값: /logout) .logoutSuccessUrl("/login") // 로그아웃 성공 시 이동할 페이지
		...
		/*
		 * httpSecurity.rememberMe(remember -> remember .key("zeus") // 인증 토큰 생성 시 사용할 키
		 * (보안상 중요) .tokenRepository(createJDBCRepository()) // DB를 이용한 토큰 저장소 설정
	 ...
	 * 
	 * 
	 * //@Autowired protected void configure(AuthenticationManagerBuilder auth)
	 * throws Exception {
	 ...
	 * 
	 * @Bean public UserDetailsService createUserDetailsService() { return new
